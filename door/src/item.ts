...
export default class Button implements IScript<Props> {
  openClip = new AudioClip('door/sounds/open.mp3')
  closeClip = new AudioClip('door/sounds/close.mp3')
  ...
  spawn(host: Entity, props: Props, channel: IChannel) {
    ...
    door.addComponent(new GLTFShape('door/models/Door_SciFi_Vertical.glb'))

    // collider
    const collider = new Entity(door.name + '-collider')
    collider.setParent(door)
    collider.addComponent(new GLTFShape('door/models/Door_SciFi_collider.glb'))
